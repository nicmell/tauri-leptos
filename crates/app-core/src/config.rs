...
pub struct AppConfig {
    /// Address the server binds in serve/tauri mode.
    pub listen: SocketAddr,
    /// Address of the API server the dev frontend proxies to.
    pub api_addr: SocketAddr,
    /// Frontend bundle directory; `None` = platform default resolution.
...
impl AppConfig {
    ...
    /// with the defaults (best effort); an unreadable or invalid file is an
    /// error — serve mode must fail fast so systemd sees it.
...
mod tests {
    ...
    fn partial_file_fills_defaults() {
        ...
        assert_eq!(parsed.api_addr, AppConfig::default().api_addr);
